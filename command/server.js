...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand =>
            subcommand
                .setName('botlist')
                .setDescription('Display all bots in the server with pagination')
                .addIntegerOption(option =>
                    option.setName('page')
                        .setDescription('Page number (default: 1)')
                        .setMinValue(1)
        ...
        try {
            switch (subcommand) {
                ...
                case 'membercount':
                    ...
                    break;
                case 'botlist':
                    await this.handleBotList(interaction);
                    break;
            }
        } catch (error) {
            console.error(error);
    ...
    async handleBotList(interaction) {
        const guild = interaction.guild;
        await guild.members.fetch();
        ...
        if (bots.size === 0) {
            await interaction.reply('This server has no bots.');
            return;
        }
        
        const totalPages = Math.ceil(bots.size / botsPerPage);
        
        if (page > totalPages) {
            await interaction.reply(`Invalid page number. This server has ${totalPages} page(s) of bots.`);
            return;
        }
        
        const startIndex = (page - 1) * botsPerPage;
        const endIndex = startIndex + botsPerPage;
        const pageBots = Array.from(bots.values()).slice(startIndex, endIndex);
        
        let description = '';
        for (let i = 0; i < pageBots.length; i++) {
            const bot = pageBots[i];
            const botNumber = startIndex + i + 1;
            description += `\`#${botNumber}.\` [${bot.user.tag}](https://discord.com/users/${bot.user.id}) [<@${bot.user.id}>]\n`;
        }
        
        const embed = new EmbedBuilder()
            .setTitle(`Bots in ${guild.name} - Page ${page}/${totalPages}`)
            .setDescription(description)
            .setFooter({ text: `Page ${page} of ${totalPages} • ${bots.size} total bots` })
            .setColor(0x00AE86);
        ...
        const row = new ActionRowBuilder()
            .addComponents(
                new ButtonBuilder()
                    .setCustomId(`botlist_first_${guild.id}`)
                    .setLabel('⏮️ First')
                    .setStyle(ButtonStyle.Secondary)
                    .setDisabled(page === 1),
                new ButtonBuilder()
                    .setCustomId(`botlist_prev_${guild.id}_${page}`)
                    .setLabel('⬅️ Previous')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(page === 1),
                new ButtonBuilder()
                    .setCustomId(`botlist_next_${guild.id}_${page}`)
                    .setLabel('Next ➡️')
                    .setStyle(ButtonStyle.Primary)
                    .setDisabled(page === totalPages),
                new ButtonBuilder()
                    .setCustomId(`botlist_last_${guild.id}_${totalPages}`)
                    .setLabel('Last ⏭️')
                    .setStyle(ButtonStyle.Secondary)
                    .setDisabled(page === totalPages)
            );
    ...
    async handleBotListButtons(interaction) {
        const guild = interaction.guild;
        await guild.members.fetch();
