...
client.on('interactionCreate', async interaction => {
    if (interaction.isChatInputCommand()) {
        ...
        // Handle serverlist pagination buttons
        else if (interaction.customId.startsWith('serverlist_')) {
            const botCommand = client.commands.get('bot');
            if (botCommand) {
                await botCommand.handleServerListButtons(interaction);
            }
        }
        // Handle botlist pagination buttons
        else if (interaction.customId.startsWith('botlist_')) {
            const serverCommand = client.commands.get('server');
            if (serverCommand) {
                await serverCommand.handleBotListButtons(interaction);
            }
        }
    }
});
